...
We next evaluate the variances for tentative biomarker proteins. LLLLL

- are this higher or lower than others?
- relative CV (CV_study / CV_QC). there is a dependency between both!
...
png("images/manuscript/Figure_2.png", width = 6.4,
...
Additionally, we evaluated the predictive ability of all plasma proteins 
for hormonal contraceptive use. To ensure comparability, we matched the users of 
hormonal contraceptives to a control group of identical size (n = 275). 
Only non-pregnant women under 40 years not taking hormonal contraceptives 
were considered for the control group.

```{r, eval = FALSE}
## Prepare Data for matching
# Filter for  HCU users and controls
filter_hcu <- ((prot_data$Age <= 40) & (prot_data$HCU == "Yes") & (prot_data$Pregnant == "No"))
filter_control <- ((prot_data$Age <= 40) & (prot_data$HCU == "No") & (prot_data$Pregnant == "No"))
...
women_hcu_control <- as.data.frame(rbind(women_hcu, women_control))

# drop unnecessary columns
drop_col <- c("File.Name", "Date", "Inst", "Inj", "Plate", "Sample.Name", "Sex", "container", "MS_Batch", "SP_Batch", "BMIcat", "Season", "Pregnant", "Age_10")
women_hcu_control <- women_hcu_control[,!(names(women_hcu_control) %in% drop_col)]
...
```{r, eval = FALSE}
# Matching
match.out <- matchit(HCU ~ Age + Fasting + BMI, data=women_hcu_control[,c("HCU", "Fasting", "BMI", "Age")], method="optimal", distance="glm")

# Select matched participants
hcu_control_matched <- rbind(women_control[match.out$match.matrix, ], women_hcu)

summary(match.out)
```

We further visualize propensity scores and covariate distributions to assess matching quality. 
Controls (HCU = No) are shown in gray - Treatment group (HCU = Yes) in black!
Propensity score evaluation and inspection of covariate distribution, show sufficient matching 
...
Predictive performance of proteins for HCU is evaluated by calculating Area Under the Receiver Operating Characteristic curve (AUROC).

```{r, eval = FALSE}
# Select only plasma protein and HCU status
hcu_control_matched <- hcu_control_matched[,c(head(prot_ann$description,148), "HCU")]
...
roc_df$Genes <- head(prot_ann$Genes,148)
roc_df$UniProtId <- head(prot_ann$description,148)

## Save the ROC data to a CSV file 
#write.csv(roc_df, "rocValsHCU.csv", row.names = FALSE)
```

Angiotensinogen (AGT) shows high predictive power for hormonal contraceptive usage (AUROC = 0.89)

```{r, eval = FALSE}
# Calculate ROC seperately for AGT
roc_data_agt <- roc(hcu_control_matched$HCU, hcu_control_matched$P0101
...
# Create ROC Curvee
roc_plot <- ggplot(roc_df, aes(x = OneMinusSpecificity, y = Sensitivity)) +
  geom_line(color = "black", size = 1) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "gray") +
  geom_text(aes(x = 0.2, y = 0.1, label = paste("AUC AGT =", round(roc_data_agt$auc,2))), 
            size = 4, color = "black", hjust = -2.0, vjust = 1.5) +
  labs(
    x = "1 - Specificity",
    y = "Sensitivity",
  ) +
...
ggsave("images/manuscript/Figure_2B.png", plot = roc_plot, width = 6, height = 4, dpi = 600)
