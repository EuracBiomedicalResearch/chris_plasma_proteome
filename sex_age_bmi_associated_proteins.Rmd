...
png("images/manuscript/supplement/Figure_S150.png", width = 6.4,
    height = 6, units = "cm", res = 600, pointsize = 4, type = "cairo-png")
library(vioplot)
par(mfrow = c(1, 1), mar = c(4.3, 4.5, 2, 0.5), cex.axis = 1.5,
    cex.lab = 1.5, bty = "n", cex.main = 2.5)
vioplot(split(prot_data$P01019, f), ylab = expression(log[2]~abundance),
        main = "P01019 (AGT)")
grid(nx = NA, ny = NULL)

## ## Distribution for SERPING1 (P05155;P05155-3)
## vioplot(split(prot_data$`P05155;P05155-3`, f),
##         ylab = expression(log[2]~abundance),
##         main = "P05155;P05155-3 (SERPING1)")
## grid(nx = NA, ny = NULL)
dev.off()
...
par(mar = c(3.5, 4.5, 1.5, 1.5), mfrow = c(1, 2), cex.axis = 1.2)
plot_volcano(results, "SexFemale", main = "Female vs Male")
grid()
...
# Sensitivity analysis: impact of interaction BMI:sex on results

In the present data set, the distribution of BMI differs between female and male
participants: women have, as expected, on average a lower BMI than men (see
violin plots below).

```{r vioplot-bmi-sex, fig.path = IMAGE_PATH}
library(vioplot)
vioplot(split(prot_data$BMI, prot_data$Sex))
grid()
```

In addition we perform a Kolmogorov-Smirnov test to see if Sex and BMI are from
...
While our linear model from the previous section considers sex and BMI, due to
the dependency between these factors, also their interaction might need to be
included into the model. We thus fit below a model including also the
interaction between sex and BMI to the abundances of each protein.

```{r sensitivity-interaction}
## make a function for LM - Model 2:
FunForLinRegInt <- function(AnalyteName, Data) {
    Analyte <- Data[, AnalyteName]
    LMReg <- lm(Analyte ~ Sex * BMIcat + Age_10 + Fasting + HCU,
                data = Data)
    res <- coef(summary(LMReg))[-1, ]
    p.Values <- data.frame(c(res[, 1],
                             res[, 4]))
...
LMRegtestsInt <- do.call(
    cbind, lapply(proteins, function(x) FunForLinRegInt(x, prot_data)))
Test_output_int <- t(LMRegtestsInt)
## Adjusting for multiple hypothesis testing
adjp <- apply(Test_output_int[, grep("p-value", colnames(Test_output_int))],
              MARGIN = 2, p.adjust, method = "bonferroni")
colnames(adjp) <- sub("value", "adj", colnames(adjp))
Test_output_int <- cbind(Test_output_int, adjp)
LMRegtestsInt_AS <- do.call(
    cbind.data.frame, lapply(proteins,
                             function(x) FunForLinRegInt(x, prot_data_AS)))
Test_output_int_AS <- t(LMRegtestsInt_AS)
EffectSize_int <- Test_output_int_AS[, grep("coef", colnames(Test_output_int_AS))]
colnames(EffectSize_int) <- sub("coef", "effect_size", colnames(EffectSize_int))

```

We do not compare p-values from the different models because of their different
degrees of freedom. Below we compare the effect sizes for all covariates between
the two models.

```{r, fig.path = IMAGE_PATH, fig.cap = "Comparison of effect sizes from the analysis without (x-axis) and with (y-axis) considering an interaction between sex and BMI."}
par(mfrow = c(1, 3))
plot(EffectSize[, "effect_size_SexFemale"],
     EffectSize_int[, "effect_size_SexFemale"],
     xlab = expression(ES[Sex]), ylab = expression(ES[Sex]^int),
     pch = 16, col = "#00000060", main = "Sex")
abline(0, 1, col = "grey", lty = 2)
grid()
plot(EffectSize[, "effect_size_BMIcat3"],
     EffectSize_int[, "effect_size_BMIcat3"],
     xlab = expression(ES[BMI3]), ylab = expression(ES[BMI3]^int),
     pch = 16, col = "#00000060", main = "BMI 3 vs 2")
abline(0, 1, col = "grey", lty = 2)
grid()
plot(EffectSize[, "effect_size_BMIcat4"],
     EffectSize_int[, "effect_size_BMIcat4"],
     xlab = expression(ES[BMI4]), ylab = expression(ES[BMI4]^int),
     pch = 16, col = "#00000060", main = "BMI 4 vs 2")
abline(0, 1, col = "grey", lty = 2)
grid()

png("images/manuscript/supplement/Figure_sensitivity_interaction.png",
    width = 12, height = 4, units = "cm", res = 600, pointsize = 4)
par(mfrow = c(1, 3), mar = c(4.5, 5, 1.5, 0.5), cex.lab = 1.5, bty = "n",
    cex.main = 2.5)
plot(EffectSize[, "effect_size_SexFemale"],
     EffectSize_int[, "effect_size_SexFemale"],
     xlab = expression(ES[Sex]), ylab = expression(ES[Sex]^int),
...
Some influence from inclusion of an interaction term between sex and BMI can be
observed for the results on sex and BMI association.

```{r sensitivity-interaction-age-hcu, fig.path = IMAGE_PATH, fig.cap = "Comparison of effect sizes from the analysis without (x-axis) and with (y-axis) interaction between sex and BMI."}
par(mfrow = c(1, 2))
plot(EffectSize[, "effect_size_Age_10"],
     EffectSize_int[, "effect_size_Age_10"],
     xlab = expression(ES[Age]), ylab = expression(ES[Age]^int),
     pch = 16, col = "#00000060", main = "Age")
abline(0, 1, col = "grey", lty = 2)
grid()

plot(EffectSize[, "effect_size_HCUYes"],
     EffectSize_int[, "effect_size_HCUYes"],
     xlab = expression(ES[HCU]), ylab = expression(ES[HCU]^int),
     pch = 16, col = "#00000060", main = "HCU")
abline(0, 1, col = "grey", lty = 2)
grid()

```

Results for age and hormonal contraceptive use are independent of the
