...
[@verri_hernandes_age_2022]. In addition, influence of medication on the
quantified plasma proteome is evaluated.

# Data import

Below we load the data and all required libraries. The data is loaded from the
CHRIS TDFF modules. Most used packages are available either on
...
Top 30 proteins with highest absolute CV in study samples.
...
Proteins with the lowest absolute CV.
...
available `r length(proteins)` proteins. Samples are coloured according to
their sex. Prior the PCA, data is normalized to 0 mean and standard deviation of
1 (autoscaling). Plot shows the scores and loadings of all subjects in the
space of the first two PCs, with 15% of total variances explained.
...
png("images/manuscript/Figure_1.png", width = 10, height = 10, units = "cm",
...
cases when angles between arrows are around 90°, indicates that there is no
...
contraceptives. We next evaluate which preparations are taken as hormonal
contraceptives.
...
Among them, G03AB08 (dienogest and estradiol) use bioidentical hormone
(estradiol valerate) and G03AC09 (desogestrel) progesteron only were described
to have more neutral effects than other combined preparations that include
ethinylestradiol [Kangasniemi et al. Human Reprod. 2023].
...
G03H <- contr[contr$atc_3 == "G03H", ]
G02B <- contr[contr$atc_3 == "G02B", ]
G03A <- contr[contr$atc_3 == "G03A", ]

table(G03H$atc_4_name)
table(G02B$atc_4_name)
table(G03A$atc_4_name)

png("contraceptives_coloring.png", width = 10, height = 10, units = "cm",
    res = 600, pointsize = 4)
cols <- rep("#99999920", nrow(prot_data))
cols[rownames(prot_data) %in% G03H$AID] <- "#FF0000FF"
cols[rownames(prot_data) %in% G02B$AID] <- "#00FF00FF"
cols[rownames(prot_data) %in% G03A$AID] <- "#0000FFFF"
par(mar = c(4.5, 4.3, 0.5, 0.5), cex.lab = 1.5, bty = "n")
plot_pca(pc, pch = NA, col = NA, bg = cols, cex = 1.5,
         xlim = c(-16, 9))
unsigned.range <- function(x)
        c(-abs(min(x, na.rm = TRUE)), abs(max(x, na.rm = TRUE)))
y <- pc$rotation[, 1:2]
...
text(y[, 1] * 1.1, y[, 2] * 1.1, labels = rownames(y),
     col = "#00000080", cex = 1.4)
points(pc$x[, 1], pc$x[, 2], pch = 21, col = NA, bg = cols,
       cex = 1.5)
grid()
dev.off()
## Summary: green ones (G02B topical use) seem split half/half.


png("contraceptives_topical_coloring.png", width = 10, height = 10,
    units = "cm", res = 600, pointsize = 4)
cols <- rep("#99999920", nrow(prot_data))
cols[rownames(prot_data) %in% G02B[G02B$atc_4 == "G02BA", "AID"]] <- "#FF0000FF"
cols[rownames(prot_data) %in% G02B[G02B$atc_4 == "G02BB", "AID"]] <- "#0000FFFF"
par(mar = c(4.5, 4.3, 0.5, 0.5), cex.lab = 1.5, bty = "n")
plot_pca(pc, pch = NA, col = NA, bg = cols, cex = 1.5,
         xlim = c(-16, 9))
unsigned.range <- function(x)
...
## Now, that's fun: blue (G02BB, intravaginal) behave like oral contraceptives,
## red (G02A, intrauterine) like "normal". Thats expected, since the
## intrauterine contraceptives are expected to act mostly locally, while the
## intravaginal mostly systemic (while by-passing the liver).

```

Additionally, we evaluated the predictive ability of all plasma proteins for
...
non-pregnant women under 40 years not taking hormonal contraceptives were
...
```{r}
## Figure 2.
png("images/manuscript/Figure_2.png", width = 10,
    height = 5, units = "cm", res = 600, pointsize = 4, type = "cairo-png")
par(mfrow = c(1, 2), mar = c(4, 4.5, 5, 1), cex.axis = 1.5, cex.lab = 1.5,
    bty = "n", cex.main = 2.5)
## Supplementary Figure P01019 AGT concentration in different groups
f <- as.character(prot_data$Sex)
...
contraceptives is included to adjust for the influence of this factor. In this
model, no interactions between variables are considered, although a relationship
between BMI and sex seems to be present in the present data set. A sensitivity
analysis evaluating the impact of considering in addition an interaction between
BMI and sex is performed in one of the following sections.
...
dir.create(dr, showWarnings = FALSE, recursive = TRUE)
png(paste0(dr, "_Figure_3ab.png"), width = 8.6, height = 4.1, units = "cm",
...
pheatmap(coefs, filename = paste0(dr, "_Figure_3c.png"),
...
```{r}
## Figure 4 paper.
png("images/manuscript/Figure_4.png", width = 12, height = 4, units = "cm",
    res = 600, pointsize = 4)
par(mfrow = c(1, 3), mar = c(4.5, 4.5, 1.5, 0.5), cex.lab = 2, bty = "n",
...
again the analysis restricting to female participants below 40 years of age to
...
par(mfrow = c(1, 3), mar = c(4.5, 4.3, 1.5, 0.5), cex.lab = 1.5, bty = "n",
    cex.main = 2.5)
x <- results$p.value_HCUYes
y <- results_hcu$p.value_HCUYes
x[x == 0] <- min(x[x > 0]) / 10
...
For the analysis, we first restrict to oral contraceptives and categorize them
into preparations that contain ethinylestradiol (EE), preparations with
bioidentical hormones (BI) and progestogen preparations (P4). In the present
data set, two preparations contain bioidentical hormones: ZOELY (estradiol (E2))
and KLAIRA (estradiol valerate (EV)).

```{r, results = "asis"}
#' Define the table with contraceptives.
oral_hcu <- data.frame(
    ATC5 = c("G03AA07",
             "G03AA09",
             "G03AA10",
...
pandoc.table(
    ...
    caption = paste0("Oral contraceptive preparations with ethinylestradiol ",
                     "(EE), bioidentical estrogen (BI) or progestogen (P4) ",
                     "in the present data set.")
)
```

To define the data set for the analysis, we restrict the data for female
participants below age of 40 to those that either take one of the oral
contraceptives listed above, or that don't take any hormonal contraceptives
(i.e. excluding all with ATC3 codes G03H (antiandrogens), G02B (contraceptives
for topical use) or G03A (contraceptives for systemic use).
...
prot_data_ohc$hcu_class[prot_data_ohc$atc_5 %in%
                        oral_hcu$ATC5[oral_hcu$hcu_class == "BI"]] <- "BI"
prot_data_ohc$hcu_class <- factor(prot_data_ohc$hcu_class,
                                  levels = c("none", "BI", "P4", "EE"))
...
Only very few or no protein was found significantly different between BI or P4
and no HCU. The number of significant proteins for EE contraceptives, in
contrast, is very high. Note that the number of significant proteins is also
dependent on the size of the different groups, and here EE is the largest group
with the strongest statistical power. Below we evaluate the effect of the
different preparations using volcano plots.
...
yl <- range(-log10(results_ohc[, c("p.adj_hcu_classEE", "p.adj_hcu_classBI",
                                   "p.adj_hcu_classP4")]))
xl <- range(results_ohc[, c("coef_hcu_classEE", "coef_hcu_classBI",
                            "coef_hcu_classP4")])

par(mfrow = c(1, 3))
y <- -log10(results_ohc$p.adj_hcu_classBI)
x <- results_ohc$coef_hcu_classBI
plot(x, y, main = "HCU: BI", xlab = "coefficient",
...
We next plot the coefficients for differential abundance of EE, BI or PI against
no HCU against each other.

```{r, fig.cap = "Comparison of coefficients representing differential abundance between EE, BI or P4 preparations against no HCU.", fig.width = 10, fig.height = 5}
a <- results_ohc$coef_hcu_classEE
b <- results_ohc$coef_hcu_classBI
d <- results_ohc$coef_hcu_classP4

par(mfrow = c(1, 2), bty = "n")
xl <- range(c(a, b, d))
plot(a, b, pch = 21, col = "#00000080", bg = "#00000040",
     xlab = expression(coef[EE]), ylab = expression(coef[BI]),
     xlim = xl, ylim = xl)
l <- lm(b ~ a)
abline(l)
...
```{r}
xl <- range(results_ohc[, c("coef_hcu_classEE", "coef_hcu_classBI")])
gg <- ggplot(results_ohc, aes(x = coef_hcu_classEE, y = coef_hcu_classBI)) +
...
Both, ethinylestradiol and bioidentical estrogen containing preparations affect
about the same plasma proteins, but the effect of ethinylestradiol is stronger
(on average about 50%). Progestogen containing preparations, on the other hand,
affect, if at all, different proteins and seem to have in general a much lower
impact on the plasma proteome.


```{r, fig.cap = "Comparison of effect sizes between EE, BI or P4 preparations against no HCU.", fig.width = 10, fig.height = 5}
a <- results_ohc$effect_size_hcu_classEE
b <- results_ohc$effect_size_hcu_classBI
d <- results_ohc$effect_size_hcu_classP4

par(mfrow = c(1, 2), bty = "n")
xl <- range(c(a, b, d))
plot(a, b, pch = 21, col = "#00000080", bg = "#00000040",
     xlab = expression(ES[EE]), ylab = expression(ES[BI]),
     xlim = xl, ylim = xl)
l <- lm(b ~ a)
abline(l)
legend("topleft", legend = paste0("slope: ", format(coef(l)[2], digits = 3)))
...
## Validation of HCU results with Somalogic data

To validate the results we check how measurements for HCU associated proteins
quantified with Scanning SWATH might relate to Somalogic-based measurements.

```{r}
soma <- data_module("chris_somalogic", "1.0.3.2", chris_path)
soma_data <- data(soma)
soma_ann <- labels(soma)
stopifnot(all(colnames(soma_data) == rownames(soma_ann)))
colnames(soma_data) <- soma_ann$description
rownames(soma_ann) <- soma_ann$description

common_aid <- intersect(rownames(soma_data), rownames(prot_data_sub))
soma_data <- soma_data[common_aid, ]
prot_data_sub2 <- prot_data_sub[common_aid, ]

```

Evaluation of measurements for AGT (P01019); very nice.

```{r}
idx <- which(soma_ann$EntrezGeneSymbol == "AGT")
soma_id <- soma_ann[idx, "description"]
prot_id <- "P01019"

x <- prot_data_sub2[, prot_id]
y <- log2(soma_data[, soma_id])
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
     main = "AGT, P01019", xlab = "Scanning SWATH", ylab = "Somalogic")
grid()
l <- lm(y ~ x)
abline(l)
summary(l)
...
Evaluation of measurements for CP (P00450); less good match.

```{r}
idx <- which(soma_ann$EntrezGeneSymbol == "CP")
soma_id <- soma_ann[idx, "description"]
prot_id <- "P00450"
...
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
     main = "CP, P00450", xlab = "Scanning SWATH", ylab = "Somalogic")
grid()
l <- lm(y ~ x)
abline(l)
summary(l)
...
Evaluation for PGLYRP2 (Q96PD5); very nice.

```{r}
idx <- which(soma_ann$EntrezGeneSymbol == "PGLYRP2")
soma_id <- soma_ann[idx, "description"]
prot_id <- "Q96PD5"
...
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
     main = "PGLYRP2, Q96PD5", xlab = "Scanning SWATH", ylab = "Somalogic")
grid()
l <- lm(y ~ x)
abline(l)
summary(l)
...
Evaluation for SERPINA6 (P08185); ah, well.

```{r}
idx <- which(soma_ann$EntrezGeneSymbol == "SERPINA6")
soma_id <- soma_ann[idx, "description"]
prot_id <- "P08185"
...
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
     main = "SERPINA6, P08185", xlab = "Scanning SWATH", ylab = "Somalogic")
grid()
l <- lm(y ~ x)
abline(l)
summary(l)
...
Evaluation for SHBG (); depending on the dilution factor good or less good.

```{r}
idx <- which(soma_ann$EntrezGeneSymbol == "SHBG")
soma_id <- soma_ann[idx[1], "description"]
prot_id <- "P04278"
...
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
     main = "SHBG, P04278", xlab = "Scanning SWATH", ylab = "Somalogic")
grid()
l <- lm(y ~ x)
abline(l)
summary(l)

soma_id <- soma_ann[idx[2], "description"]
x <- prot_data_sub2[, prot_id]
y <- log2(soma_data[, soma_id])
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
...
Evaluation for FETUB (Q9UGM5); ok-ish.

```{r}
idx <- which(soma_ann$EntrezGeneSymbol == "FETUB")
soma_id <- soma_ann[idx, "description"]
prot_id <- "Q9UGM5"
...
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
     main = "FETUB, Q9UGM5", xlab = "Scanning SWATH", ylab = "Somalogic")
grid()
l <- lm(y ~ x)
abline(l)
summary(l)
...
Evaluation for PLG (P00747); depending on the used dilution.

```{r}
idx <- which(soma_ann$EntrezGeneSymbol == "PLG")
prot_id <- "P00747"
...
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
     main = "PLG, P00747", xlab = "Scanning SWATH", ylab = "Somalogic")
grid()
l <- lm(y ~ x)
abline(l)
summary(l)
...
soma_id <- soma_ann[idx[3], "description"]
x <- prot_data_sub2[, prot_id]
y <- log2(soma_data[, soma_id])
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
...
Evaluation for GC (Q9UGM5); good.

```{r}
idx <- which(soma_ann$EntrezGeneSymbol == "GC")
soma_id <- soma_ann[idx, "description"]
prot_id <- "P02774;P02774-3"

x <- prot_data_sub2[, prot_id]
y <- log2(soma_data[, soma_id])
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
     main = "GC, P02774", xlab = "Scanning SWATH", ylab = "Somalogic")
grid()
l <- lm(y ~ x)
abline(l)
summary(l)
...
Evaluation for SERPINA1 (P01009); good.

```{r}
idx <- which(soma_ann$EntrezGeneSymbol == "SERPINA1")
soma_id <- soma_ann[idx, "description"]
prot_id <- "P01009"
...
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
     main = "SERPINA1, P01009", xlab = "Scanning SWATH", ylab = "Somalogic")
grid()
l <- lm(y ~ x)
abline(l)
summary(l)
...
Evaluation for KNG1 (P01042;P01042-2); choose what you like.

```{r}
idx <- which(soma_ann$EntrezGeneSymbol == "KNG1")
prot_id <- "P01042;P01042-2"
...
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
     main = "KNG1, P01042", xlab = "Scanning SWATH", ylab = "Somalogic")
grid()
l <- lm(y ~ x)
abline(l)
summary(l)
...
soma_id <- soma_ann[idx[4], "description"]
x <- prot_data_sub2[, prot_id]
y <- log2(soma_data[, soma_id])
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
...
Evaluation for SERPINC1 (P01008); good.

```{r}
idx <- which(soma_ann$EntrezGeneSymbol == "SERPINC1")
soma_id <- soma_ann[idx, "description"]
prot_id <- "P01008"
...
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
     main = "SERPINC1, P01008", xlab = "Scanning SWATH", ylab = "Somalogic")
grid()
l <- lm(y ~ x)
abline(l)
summary(l)
...
Evaluation for LUM (P51884); good.

```{r}
idx <- which(soma_ann$EntrezGeneSymbol == "LUM")
soma_id <- soma_ann[idx, "description"]
prot_id <- "P51884"
...
plot(x, y,
     pch = 21, col = "#00000080", bg = "#00000040",
     main = "LUM, P51884", xlab = "Scanning SWATH", ylab = "Somalogic")
grid()
l <- lm(y ~ x)
abline(l)
summary(l)
...
Checking the protein mentioned by reviewer 1 (LPA/apolipoprotein a) - for
Somalogic we have however the different isoforms APOA1, APOA2, APOA4, APOA5.

```{r}
## idx <- which(soma_ann$EntrezGeneSymbol == "APOA")
## soma_id <- soma_ann[idx, "description"]
## prot_id <- "P08519"
## idx <- which(soma_ann$UniProt == prot_id)
...
ggsave("images/manuscript/Figure_ATC3.png", plot = plt, dpi = 600, width = 12,
...
```{r, fig.cap = "Age distribution for women below 40 in the various HCU categories."}
...
```{r, fig.cap = "Abundance of AGT in women (below age of 40) in the various HCU categories."}
...
contraceptives or that did previously take contraceptives. This analysis did
however not take the last time hormonal contraceptives were taken into account.
...
never used them. This analysis is performed on female participants of age below
...
par(mfrow = c(1, 3), mar = c(5, 5, 2, 0.5), cex.lab = 2, bty = "n",
...
grid()
vioplot(split(women_data_40$P01019, women_data_40$HCU_history),
        main = "AGT", ylab = expression(log[2]~abundance))
grid()
dev.off()
...
```{r, fig.cap = "Distribution of estimated HCU free period for women (below 40) with past HCU."}
...
plot(density(women_data_40_past$eHCU_free_period, na.rm = TRUE),
     ...
     main = "Women below 40 with previous HCU")
```
...
```{r, fig.cap = "Relationship between AGT plasma concentrations and HCU free time for women below 40 with past HCU."}
