...
We now consider clinical and lab parameters from the CHRIS datasets. We restrict 
the data set to only numeric lab or clinical parameters and in addition remove 
parameters with more than 50% of missing values.

```{r}
## annotation key
NamesKey <- chrisTraitAnnotations()
## merge lab & clin chris datasets
chris_cl <- cbind.data.frame(labdata_info[rownames(prot_data), -1],
                             clin[rownames(prot_data), -1])
## select numeric variables
chris_cl <- chris_cl[, NamesKey[colnames(chris_cl), "type"] == "numeric"]
chris_cl <- chris_cl[, vapply(chris_cl, is.numeric, logical(1))]
chris_cl <- chris_cl[, vapply(
    chris_cl, function(z) sum(is.na(z)) < length(z) / 2, logical(1))]
```

Compute Spearman correlation coefficients between clinical/lab and SWATH
proteins:

```{r, warnings = FALSE}
cormat <- cor(chris_cl, prot_data[, proteins],
              method = "spearman", use = "pairwise.complete.obs")
# Melt the correlation matrix
prot_cors <- melt(cormat, na.rm = TRUE)
colnames(prot_cors) <- c("label", "UniProt", "SWATH_rho")
prot_cors$label <- as.character(prot_cors$label)
prot_cors$UniProt <- as.character(prot_cors$UniProt)
prot_cors$gene <- prot_ann[prot_cors$UniProt, "Genes"]
prot_cors$description <- prot_ann[prot_cors$UniProt, "long_description"]
prot_cors$name <- NamesKey[prot_cors$label, "name"]
prot_cors$note_detail <- NamesKey[prot_cors$label, "note_detail"]
prot_cors <- prot_cors[abs(prot_cors$SWATH_rho) > 0.2, ]
prot_cors$SWATH_rho <- round(prot_cors$SWATH_rho, 6)
prot_cors <- prot_cors[order(abs(prot_cors$SWATH_rho), decreasing = TRUE), ]
prot_cors %>%
  DT::datatable(
      extensions = 'Buttons', 
...
Correlation for the best correlating pair is shown below.

```{r, fig.path = IMAGE_PATH, fig.cap = "Best correlating protein and lab parameter.", fig.height = 7, fig.width = 7}
plot(prot_data[, prot_cors$UniProt[1L]],
     chris_cl[, prot_cors$label[1L]],
     pch = 16, col = "#00000060",
     xlab = prot_cors$gene[1L],
     ylab = prot_cors$name[1L],
     main = "ScanningSWATH")
grid()
```
