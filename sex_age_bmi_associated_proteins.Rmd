...
FunForLinReg <- function(AnalyteName, Data) {
    ...
    LMReg <- lm(Analyte ~ Sex + Age + Fasting + BMIcat + HCU,
                data = Data)
...
grid()
## abline(h = -log10(0.05), lty = 2)
significant_points(results, sign_es, comparison = "Age", cex = 1.1)
dev.off()
...
Since hormonal contraceptive use is clearly related with sex, we perform the
analysis on the subset of female participants only. To avoid any potential age
bias we further subset the data to participants below 40 years of age. In this
subset we fit linear models to the data explaining protein abundances by the
age, BMI, fasting status and hormonal contraceptive use.
...
FunForLinRegHCU <- function(AnalyteName, Data) {
    Analyte <- Data[, AnalyteName]
    LMReg <- lm(Analyte ~ Age + Fasting + BMIcat + HCU,
                data = Data)
    res <- coef(summary(LMReg))[-1, ]
    p.Values <- data.frame(c(res[, 1],
                             res[, 4]))
...
LMRegtests <- do.call(
    cbind, lapply(proteins, function(x) FunForLinRegHCU(x, prot_data_sub)))
...
sig_tab <- significant_table(
    results_hcu, "HCUYes",
    columns = c("Genes", "Description", "cv_qc_chris", "coef_HCUYes",
                "p.adj_HCUYes"))
## Exporting results
...
tab <- cbind(results_sens[sel, c("Genes", "coef_Age", "p.adj_Age")],
             results[sel, c("coef_Age", "p.adj_Age",
                            "coef_HCUYes", "p.adj_HCUYes")])
tab <- tab[order(tab[, 3]), ]
pandoc.table(
    tab, style = "rmarkdown", split.table = Inf,
