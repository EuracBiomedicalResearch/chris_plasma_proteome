...
#' General settings
filename <- "sex_age_bmi_associated_proteins_int"
...
FunForLinReg <- function(AnalyteName, Data) {
    ...
    LMReg <- lm(Analyte ~ Age_10 + Fasting + BMIcat * Sex + HCU,
                data = Data)
...
FunForLinReg2 <- function(AnalyteName, Data) {
    Analyte <- Data[, AnalyteName]
    LMReg <- lm(Analyte ~ Age_10 + Fasting + BMIcat * Sex,
                data = Data)
    res <- coef(summary(LMReg))[-1, ]
    p.Values <- data.frame(c(res[, 1],
                             res[, 4]))
