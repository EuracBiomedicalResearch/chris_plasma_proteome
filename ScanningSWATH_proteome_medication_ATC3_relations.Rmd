---
title: "Relationships between ScanningSWATH mass-spectrometry (MS)-based plasma proteome and medication status (ATC 3 and ATC 4)" 
...
# Evaluating medication status (ATC 3) and his impact on protein concentrations
...
matR <- matrix(0, nrow = length(proteins), ncol = 27)
...
## cluster values in matrix
clust1 <- hclust(dist((matR)), method = "ward.D")
clust2 <- hclust(dist(t(matR)), method = "ward.D")
...
meltED %>% 
      ...
      geom_text(size = 2.5) +
      theme_classic() +
      scale_x_discrete(expand=c(0,0), limits = colnames(matR2)[clust2$order]) +
      scale_y_discrete(expand=c(0,0), limits = rownames(matR2)[clust1$order]) +
...
meltED %>% 
      ...
      theme_classic() +
      scale_x_discrete(expand=c(0,0), limits = colnames(matR2)[clust2$order]) +
      scale_y_discrete(expand=c(0,0), limits = prot_ann[match(rownames(matR2), 
      rownames(prot_ann)), "long_description"][clust1$order]) +
      theme(text = element_text(family = "Roboto"), 
      axis.text.x = element_text(angle = 15, hjust = 1, size = 7), 
      axis.text.y = element_text(hjust = 1, size = 7))
```
...
FM2 <- FM[rowSums(FM)!= 0,]
clust1 <- hclust(dist((FM2)), method = "ward.D")
clust2 <- hclust(dist(t(FM2)), method = "ward.D")
...
melted %>% 
      ...
      geom_text(size = 2.5) +
      theme_classic() +
      scale_x_discrete(expand=c(0,0), limits = colnames(FM2)[clust2$order]) +
      scale_y_discrete(expand=c(0,0), limits = rownames(FM2)[clust1$order]) +
      theme(text = element_text(family = "Roboto"), 
      axis.text.x = element_text(angle = 15, hjust = 1, size = 7), 
      axis.text.y = element_text(hjust = 1, size = 7))  
```

Hierarchical clustering with "ward.D" method bring all 3 contraceptives together!
The same figure but with protein descriptions on y-axis:
...
melted %>% 
      ...
      theme_classic() +
      scale_x_discrete(expand=c(0,0), limits = colnames(FM2)[clust2$order]) +
      scale_y_discrete(expand=c(0,0), limits = prot_ann[match(rownames(FM2), 
      rownames(prot_ann)), "long_description"][clust1$order]) +
      theme(text = element_text(family = "Roboto"), 
      axis.text.x = element_text(angle = 15, hjust = 1, size = 7), 
      axis.text.y = element_text(hjust = 1, size = 7))
...
# Evaluating medication status (ATC 4) and his impact on protein concentrations

We next analyze all medication taken by subjects in our analyses to determine if 
protein levels are affected by some medication intake (ATC 4).
Subset of data where medications are taken up to 2 times per week is considered 
excluding where medications are taken less frequent.
...
We're next counting for each ATC level 4 medication the number of (distinct)
participants and showing a table with the summary of medication applied to at 
least 15 subjects.

```{r, results = "asis"}
level4 <- split(medi$AID, medi$atc4)
level4 <- lapply(level4, unique)
level4_sub <- split(medi_sub$AID, medi_sub$atc4)
level4_sub <- lapply(level4_sub, unique)   
medi_sub_summary <- data.frame(medication = names(level4_sub),
                              count = lengths(level4_sub))
tab <- medi_sub_summary[medi_sub_summary$count > 14, ]
tab <- tab[order(tab$count, decreasing = TRUE), ]
rownames(tab) <- NULL
pandoc.table(tab, style = "rmarkdown", split.tables = Inf,
      caption = "Most frequent medication applied to chrism population (ATC 4)")
```

We also evaluate the overlap of subjects taking multiple medications.

```{r overlap-medication-atc4, fig.path = IMAGE_PATH, fig.cap = "Medication overlap", echo = FALSE}
overlap_matrix <- matrix(0, ncol = nrow(tab),
                         nrow = length(unique(unlist(level4_sub))))
rownames(overlap_matrix) <- unique(unlist(level4_sub))
colnames(overlap_matrix) <- tab$medication
for (n in colnames(overlap_matrix)) {
    overlap_matrix[level4_sub[[n]], n] <- 1
}
overlap_matrix <- cbind(overlap_matrix, L4 = rep(0, nrow(overlap_matrix)))
library(UpSetR)
upset(as.data.frame(overlap_matrix), nset = ncol(overlap_matrix))
...
Considering selected 29 medication groups, seems that there is little overlap 
between medications, and most of participants take one medication.

```{r}
Tab <- unlist(lapply(tab$medication, 
                              function(treat) level4_sub[[treat]]))
```

There are `r length(unique(Tab))` subjects which takes any medication 
(considering 29 medications groups) whhich is  
`r round((length(unique(Tab))/nrow(prot_data))*100, 2)` % of our subjects.
...
In this analysis, all medications are considered simultaniusly!

To identify proteins affected with different medication therapies, linear 
regression models are fitted for all proteins separately (as response) and sex, 
age, fasting status, BMI and  medication treatment (all together-approach 2) as 
a covariates. 
Model coefficients and p-values are provided. Raw p-values are adjusted for 
multiple hypothesis testing with the method from Bonferroni.
Additionally, to obtain comparable coefficients, prior to linear modelling all 
...
First, variables are created for top 29 medication groups. Summary info is 
presented: 

```{r function-medication-classes-atc4}
## make function to define class of affected subjects with medications
ClassFunction <- function(treat)  {
Class <- rep("No", nrow(prot_data))
Class[rownames(prot_data) %in% level4_sub[[treat]]] <- "Yes"
return(Class)
}
...
Medication info from all 29 groups are used in linear regression equation!
...
prot_data_AS[, proteins] <- scale(prot_data_AS[, proteins], scale = TRUE)
## do it in one run
LMRegtests_AS3 <- do.call(cbind.data.frame,
                         lapply(proteins,
                                function(x) FunForLinReg2(x, prot_data_AS)))
Test_output_AS3 <- t(LMRegtests_AS3)
EffectSize <- Test_output_AS3[, grep("coef", colnames(Test_output_AS3))]
colnames(EffectSize) <- sub("coef", "effect_size", colnames(EffectSize))
```
...
```{r significant-effect-size-atc4}
comps <- colnames(Test_output3)[grep("coef", colnames(Test_output3))]
sign_p <- Test_output3[, grep("p-adj", colnames(Test_output3))] < 0.05
colnames(sign_p) <- sub("p-adj", "significant", colnames(sign_p))
## Define significant analytes based on effect size.
cut_effect_size <- 0.5
sign_es <- sign_p &
...
```{r export-results-atc4, warning = FALSE}
## Loading also the information whether an analyte was already identified
## before to be significant.
results <- cbind.data.frame(
    prot_ann[proteins, c("description", "long_description", "PeptideNN",
...
The (static) table with the significant proteins for all 29 medication groups 
are shown below:
...
medicGroup <-  c(
 "`Thyroid hormones`Yes", "`Progestogens and estrogens, fixed combinations`Yes", 
 "`Platelet aggregation inhibitors excl. heparin`Yes", 
 "`HMG CoA reductase inhibitors`Yes", "`ACE inhibitors, plain`Yes", 
 "`Beta blocking agents, selective`Yes", 
 "`Angiotensin II antagonists and diuretics`Yes", 
 "`Selective serotonin reuptake inhibitors`Yes", "`Proton pump inhibitors`Yes", 
 "`Dihydropyridine derivatives`Yes", "`Angiotensin II antagonists, plain`Yes", 
 "`Calcium, combinations with vitamin D and/or other drugs`Yes", 
 "`ACE inhibitors and diuretics`Yes", "`Benzodiazepine derivatives`Yes", 
 "`Alpha-adrenoreceptor antagonists`Yes", "BiguanidesYes", 
 "`Other antidepressants`Yes", "`Vitamin K antagonists`Yes", 
 ...
 "`Intrauterine contraceptives`Yes", "`Propionic acid derivatives`Yes", 
 "`Beta blocking agents`Yes", "`Intravaginal contraceptives`Yes", 
 "`Other antiepileptics`Yes", "GlucocorticoidsYes", 
...
## do it in one run for all medications! 
AllResults2 <- do.call(cbind.data.frame, lapply(medicGroup, 
                                                function(x) TableFunct(x)))
#print(AllResults2)                                              
```
